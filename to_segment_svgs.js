...
for (const [bitstring, filepath] of TABLE) {
    const doc = new jsdom_1.default.JSDOM(fs_1.default.readFileSync("template.svg")).window.document;
    for (let i = 1; i <= 10; i++) {
    ...
    fs_1.default.writeFileSync(`glyphs/${filepath}.svg`, `<?xml version="1.0" encoding="UTF-8" standalone="no"?>${resulting_svg}`);
}
