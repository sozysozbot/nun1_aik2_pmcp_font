...
generateFonts({
  ...
  fs.readFile(`${out_path}/nun1aik2.html`, 'utf8', function (err,data) {
    if (err) {
      return console.log(err);
    }
    const result = replace_problematic_css(data);
    fs.writeFile(`${out_path}/nun1aik2.html`, result, 'utf8', function (err) {
       if (err) return console.log(err);
    });
  });

  fs.readFile(`${out_path}/nun1aik2.css`, 'utf8', function (err,data) {
    if (err) {
      return console.log(err);
    }
    const result = replace_problematic_css(data);
    fs.writeFile(`${out_path}/nun1aik2.css`, result, 'utf8', function (err) {
       if (err) return console.log(err);
    });
  });
